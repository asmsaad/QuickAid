...
const CustomInput = (props) => {
    const { title, field_suggetions, field_suggetions_ignore, data_update_at, data_update_var } = props;
    const { newEntryInputData, setNewEntryInputData } = data_update_at; //* Main State to store data

    return (
        <Box sx={{ width: "100%" }}>
            {title && <InfoLabel style={{ margin: "10px 0 5px 0" }}>{title}</InfoLabel>}
            <Input
                style={{ width: "100%" }}
                // allowClear
...
export const CustomModal = (props) => {
    ...
    return (
        <Modal title={title} open={isModalOpen} onOk={handleOk} onCancel={handleCancel} footer={null} maskClosable={false}>
            <Box sx={{ width: "100%", display: "flex", flexDirection: "column", gpa: "10px" }}>
                    ...
                    return (
                        <Box key={index} style={{ width: "100%" }}>
                            <CustomInputField
                                ...
                                // placeholder={modalBodyConfig[key_name]["data_update_var"]}
                ...
                <Box sx={{ width: "100%", display: "flex", justifyContent: "flex-end", marginTop: "20px" }}>
                    <Button
                        ...
                        // onClick={() => refetch_getUpdateTicketRatingByRequeser()}
